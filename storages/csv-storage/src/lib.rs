...
impl CsvStorage {
    ...
    fn fetch_schema(&self, chain_name: &str,  table_name: &str) -> Result<Option<(Schema, bool)>> {
        ...
        if !schema_path.exists() {
            ...
            let schema = Schema {
                ...
                engine: None,
                comment: None,
                chain_name: Some(chain_name.to_owned()),
            };
    ...
    fn scan_data(&self, chain_name: &str, table_name: &str) -> Result<(Option<Vec<String>>, RowIter)> {
        let data_path = self.data_path(table_name);
        let (schema, generated) = match (self.fetch_schema(chain_name,table_name)?, data_path.exists()) {
